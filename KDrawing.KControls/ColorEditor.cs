    ...
    public partial class ColorEditor : UserControl, IColorEditor
    {
        ...
        protected virtual void ResizeComponents()
        {
            try
            {
                ...
                hexTextBox.Location = new Point(rColorBar.Location.X + 5, hexTextBox.Location.Y + 3);
                hexTextBox.Width = rColorBar.Width - 12;
                hexTextBox.FlatStyle = FlatStyle.Flat;
                hexTextBox.Font = new Font("Segoe UI", 7.8F, FontStyle.Regular, GraphicsUnit.Point, ((byte)(0)));
                this.Invalidate();

                //btnCopyHex
                btnCopyHex.Width = rNumericUpDown.Width;
                btnCopyHex.Height = hexTextBox.Height;
                btnCopyHex.Location = new Point(rNumericUpDown.Location.X, hexTextBox.Location.Y);

                this.Controls.Add(btnCopyHex);
        ...
        {
            Rectangle rect = new Rectangle(hexTextBox.Location.X - 1, hexTextBox.Location.Y - 1, hexTextBox.Width + 2, hexTextBox.Height + 2);
        ...
        private void btnCopyHex_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                rtn = "#" + this.Color.R.ToString("X2") + this.Color.G.ToString("X2") + this.Color.B.ToString("X2");
                Clipboard.SetText(rtn);
            }
            catch (Exception exprr) { MessageBox.Show(exprr.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
